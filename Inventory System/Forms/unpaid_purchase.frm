...
Begin VB.Form unpaid_purchase 
   BackColor       =   &H00FFFFFF&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Unpaid Purchase Amount"
   ClientHeight    =   4665
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5955
   Icon            =   "unpaid_purchase.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   4665
   ...
   Begin LVbuttons.LaVolpeButton cmd 
      ...
      EMBOSSS         =   16777215
      MPTR            =   0
      MICON           =   "unpaid_purchase.frx":0E42
      ALIGN           =   1
      IMGLST          =   "(None)"
      IMGICON         =   "(None)"
      ICONAlign       =   0
   ...
   End
End
Attribute VB_Name = "unpaid_purchase"
...
If Check1.Value = 1 Then
        ...
        If Check1.Enabled = True Then
            ...
            ADD_EXPENSE.Open "SELECT * FROM INCOME", db, adOpenDynamic, adLockOptimistic
            ADD_EXPENSE.AddNew
            Dim ld As New ADODB.Recordset
            ld.Open "select * from AMT_UNPAID_REMIND where TRAN_TYPE='PURCHASE' AND PARTY_NAME='" & Combo1.Text & "'", db, adOpenDynamic, adLockOptimistic
            ld.MoveLast
            ADD_EXPENSE.Fields(0).Value = Format(ld.Fields(1).Value, "dd-MMM-yyyy")
            ADD_EXPENSE.Fields(1).Value = "Discounted Amount(Purchase)"
        ...
        End If
            ...
            REM_CUST.Open "SELECT * FROM AMT_UNPAID_REMIND WHERE TRAN_TYPE='PURCHASE' AND PARTY_NAME='" & Combo1.Text & "'", db, adOpenDynamic, adLockOptimistic
...
ElseIf Check1.Value = 0 Then
            ...
            up_CUST.Open "SELECT * FROM AMT_UNPAID_REMIND WHERE TRAN_TYPE='PURCHASE' AND PARTY_NAME='" & Combo1.Text & "'", db, adOpenDynamic, adLockOptimistic
            Dim total As Double
...
cust_name.Open "select distinct PARTY_NAME from AMT_UNPAID_REMIND where TRAN_TYPE='PURCHASE'", db, adOpenKeyset, adLockOptimistic
If cust_name.RecordCount = 0 Then
    MsgBox "No Unpaid Party Found ...", vbInformation, "No Unpaid Amount Found ..."
    Unload Me
    Exit Sub
End If
...
rs.Open "select DAT,INVOICE_NO,AMT_UNPAID from GRID_PURCHASE_UNPAID where PARTY_NAME='" & Combo1.Text & "'", db, adOpenDynamic, adLockOptimistic
Set DataGrid1.DataSource = rs
Dim R As New ADODB.Recordset
R.Open "SELECT SumOfAMT_UNPAID FROM QRY_UNPAID_REPORT WHERE PARTY_NAME='" & Combo1.Text & "' AND TRAN_TYPE='PURCHASE'", db, adOpenDynamic, adLockOptimistic
