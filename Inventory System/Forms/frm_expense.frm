...
Begin VB.Form frm_expense 
   ...
   Caption         =   "Expense Data Entry From"
   ClientHeight    =   2640
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5970
   Icon            =   "frm_expense.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
...
R.Open "select * from expense_types", db, adOpenDynamic, adLockOptimistic
Combo1.Clear
While R.EOF <> True
...
If ST = True Then
    Dim R As New ADODB.Recordset
    R.Open "select * from expense_types where expense_types='" & Combo1.Text & "'", db, adOpenDynamic, adLockOptimistic
    R.Delete
    R.Close
End If
End Sub
...
frm_expense_types.Show vbModal
End Sub
...
Private Sub LaVolpeButton3_Click()
    ...
    End If
    If Len(Combo1.Text) = 0 Then
        MsgBox "Expense Type Not Selected ...", vbCritical, "Enter Proper Data ..."
        Exit Sub
    End If
    ...
    add_ex.Open "select * from EXPENSE", db, adOpenDynamic, adLockOptimistic
